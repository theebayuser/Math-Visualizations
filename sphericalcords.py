...
class SphericalCoordinateSystem(ThreeDScene):
    def construct(self):
        # 1. Setup & Title
        # Visually engaging title with mathbb font and gradient
        title = MathTex(
        ...
        self.camera.frame_center = [0, 0, -0.2]  # Negative Z moves figure UP in viewport
        ...
        # Add title and create axes simultaneously
        ...
        # Billboard the 3D label by adding it fixed_in_frame at projected position? 
        # Simpler approach: Just add it to scene, it will face camera in 2D sense
        # To make it readable, we rotate it to face camera or just leave as is.
        r_label.rotate(90*DEGREES, axis=RIGHT).rotate(90*DEGREES, axis=OUT) # Rough alignment
        ...
        # Fix P label orientation to face camera (approx)
        # Rotate 90 deg around RIGHT to stand up
        p_label.rotate(90*DEGREES, axis=RIGHT)
        # Rotate around Z to face camera (45 deg)
        p_label.rotate(45*DEGREES, axis=OUT) 

        self.play(Create(r_line), FadeIn(dot), run_time=1)
        self.add(r_label, p_label) # Add labels statically

        # Projection and Theta (Azimuth)
        # Dashed line dropping down
        ...
        # 5. Theta (Azimuth) Animation - Sweeping
        # Use projection length for the theta sweep to match geometry
        proj_len = np.linalg.norm(proj)
        
        # Initial segment on X-axis
        # Using Line instead of Line3D for smoother animation of auxiliary lines
        ...
        # Add Theta Label and Shaded Sector
        ...
        # 6. Phi (Polar) Animation - Sweeping
        # "Copy of segment r" sweeping from Z-axis
        # Use full r_val length here as it sweeps the actual radius vector
        
        # Initial segment on Z-axis
        ...
        # Add Phi Label and Arc
        # Phi is measured from +Z axis down to radius vector
        # Create an arc in 3D that shows the angle
        # We'll create points along the arc from z-axis to the point
        arc_radius = 1.2
        num_points = 20
        arc_points = []
        ...
        # Create filled sector by connecting origin to arc points
        ...
        # Pivot label to look good
        phi_text.rotate(90*DEGREES, axis=RIGHT)
        
        self.play(FadeIn(phi_sector), Create(phi_arc), Write(phi_text), rate_func=smooth)

        # 7. Formulas overlay with highlighting as we write
        # Physics convention: x = rho sin(phi) cos(theta), z = rho cos(phi)
        # Create equations with separate parts for highlighting
        ...
        group = VGroup(box, formulas).to_edge(DOWN, buff=2.0) # Move up to make room for second box
        
        # Add as 2D overlay
        self.add_fixed_in_frame_mobjects(group)
        
        # Write all equations with box appearing together first
        self.play(
            FadeIn(box),
            Write(formulas),
            run_time=1.5
        ...
        # Then highlight individual terms in each equation
        # First equation - highlight rho, phi, theta in both figure and equation
        self.play(
            Indicate(r_line, scale_factor=1.1, color=GOLD),
            Indicate(r_label, scale_factor=1.3, color=GOLD),
            Indicate(eq1[2], scale_factor=1.3, color=GOLD),  # rho in eq1
            Indicate(phi_text, scale_factor=1.3, color=PINK),
            Indicate(eq1[4], scale_factor=1.3, color=PINK),  # phi in eq1
            Indicate(theta_text, scale_factor=1.3, color=RED),
            Indicate(eq1[6], scale_factor=1.3, color=RED),  # theta in eq1
            run_time=1
        )
        
        # Second equation - highlight rho, phi, theta
        self.play(
            Indicate(r_line, scale_factor=1.1, color=GOLD),
            Indicate(r_label, scale_factor=1.3, color=GOLD),
            Indicate(eq2[2], scale_factor=1.3, color=GOLD),  # rho in eq2
            Indicate(phi_text, scale_factor=1.3, color=PINK),
            Indicate(eq2[4], scale_factor=1.3, color=PINK),  # phi in eq2
            Indicate(theta_text, scale_factor=1.3, color=RED),
            Indicate(eq2[6], scale_factor=1.3, color=RED),  # theta in eq2
            run_time=1
        )
        
        # Third equation - highlight rho, phi
        self.play(
            Indicate(r_line, scale_factor=1.1, color=GOLD),
            Indicate(r_label, scale_factor=1.3, color=GOLD),
            Indicate(eq3[2], scale_factor=1.3, color=GOLD),  # rho in eq3
            Indicate(phi_text, scale_factor=1.3, color=PINK),
            Indicate(eq3[4], scale_factor=1.3, color=PINK),  # phi in eq3
            run_time=1
        )
        ...
        # 8. Pythagorean Theorem Formula (Static)
