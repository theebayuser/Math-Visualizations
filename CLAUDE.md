...
- **Text labels added before a later filled shape can get washed out** (e.g.
  a shared-edge label placed early, then a big-fill polygon added on top of
  it afterward in z-order). Manim draws by insertion order, not layering
  rules, so call `.set_z_index(n)` on labels that must stay legible above
  fills added later (see `pythagproof.py`'s `side_label` helper).
- **Nested nearby text elements need an explicit vertical gap check.** When
  stacking a caption above a bordered equation box (a very common pattern
  here), compute both elements' half-heights and verify the gap before
