...
class ExtendedDerivativeMatchCut(Scene):
    def construct(self):
        ...
        # Expanded background mathematical expressions for denser effect
        self.background_expressions = [
            ...
            # Integrals (related to derivatives)
            ...
            # Greek letters common in calculus  
        ...
        # Organize expressions by notation type
        lagrange_basic = [
        ...
        for section_name, expressions, color, notation_type in sections:
            for i, expr in enumerate(expressions):
                ...
                # Clear previous background and add new dense one
                self.clear_and_add_dense_background(current_background)
                
                if current is None:
                    # First expression overall
                    current = next_derivative
                    self.play(DrawBorderThenFill(current), run_time=1.2)
                    self.wait(0.3)
                else:
                    # Match cut with symbol alignment within same notation type
        ...
        # Transform to showcase both main notation types
    ...
    def align_derivative_symbols(self, current_expr, next_expr, notation_type):
        ...
        try:
            if notation_type == "prime":
                # For Lagrange notation, try to align the prime symbols
                current_bbox = current_expr.get_bounding_box()
                next_bbox = next_expr.get_bounding_box()
                
                # Align based on the right edge where primes typically appear
                current_right = current_bbox[1][0]  # Right edge x-coordinate
                next_right = next_bbox[1][0]
                
                # Adjust so prime positions align
                offset_x = current_right - next_right
                next_expr.shift(RIGHT * offset_x * 0.3)  # Subtle alignment
                
            elif notation_type == "fraction":
                # For Leibniz notation, align the d/dx fraction structure
                current_left = current_expr.get_left()
                next_left = next_expr.get_left()
                
                # Align the left edges where 'd' symbols start
                offset = current_left - next_left
                next_expr.shift(offset * 0.5)  # Subtle alignment for d symbols
                
            elif notation_type == "partial":
                # For partial derivatives, align the ∂ symbols
                current_left = current_expr.get_left()
                next_left = next_expr.get_left()
                
                offset = current_left - next_left
                next_expr.shift(offset * 0.4)  # Align partial symbols
                
            elif notation_type == "special":
                # For special notation, just ensure centering
                next_expr.move_to(ORIGIN)
                
            # Always ensure final result is reasonably centered
            final_center = next_expr.get_center()
            if abs(final_center[0]) > 0.5 or abs(final_center[1]) > 0.5:
                next_expr.move_to(ORIGIN)
                
        except Exception:
            # Fallback to perfect centering if alignment fails
            next_expr.move_to(ORIGIN)
    ...
    def clear_and_add_dense_background(self, current_background):
        ...
        # Add much denser background - 20-30 expressions per transition
        num_expressions = random.randint(25, 35)
        
        for _ in range(num_expressions):
            # Choose random calculus expression
...
class DerivativeNotationSections(Scene):
    def construct(self):
        # Alternative version showing clear section transitions with dense backgrounds
        self.camera.background_color = BLACK
        
        # Background calculus expressions for this scene
        self.background_expressions = [
        ...
        for section_name, section_data in sections.items():
            ...
            # Add dense background symbols for this section
            self.add_dense_background_symbols(background_mobjects)
            
            # Show characteristic symbol for this section
            if current is None:
        ...
        # Show all three notation types together
    ...
    def add_dense_background_symbols(self, background_mobjects):
        ...
        num_symbols = random.randint(8, 15)  # Much more symbols
        ...
        for _ in range(num_symbols):
            ...
            try:
                ...
                # Position in horizontal center third
                x_pos = random.uniform(-2.5, 2.5)  # Center third horizontally
                y_pos = random.uniform(-3.5, 3.5)
                
                # Avoid center where main content is
                ...
                bg_symbol.set_opacity(0.7)  # Fixed 70% opacity
                
                # Add some rotation for visual interest
                if random.random() < 0.25:
                    bg_symbol.rotate(random.uniform(-0.15, 0.15))
                
                self.add(bg_symbol)  # Add without animation for speed
    ...
    def fade_background_symbols(self, background_mobjects):
        ...
        if len(background_mobjects) > 8:
            symbols_to_remove = background_mobjects[:5]  # Remove more at once
            for mob in symbols_to_remove:
                self.remove(mob)
                background_mobjects.remove(mob)

# To render these animations:
# manim -pql filename.py ExtendedDerivativeMatchCut
# manim -pql filename.py DerivativeNotationSections
#
# For high quality:
# manim -pqh filename.py ExtendedDerivativeMatchCut