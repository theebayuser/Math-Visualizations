...
class TelescopingSeriesReel(Scene):
    ...
    def construct(self):
        # Use dark background for vibrant contrast
        self.camera.background_color = BLACK  # type: ignore[attr-defined]
        ...
        ######################
        # 3) Expand first few terms & animate cancellations
        ######################
        ...
        # Transform the general series into the expanded form
        self.play(TransformMatchingTex(series_general.copy(), expanded_terms, run_time=1.1))
        self.wait(0.3)

        # Highlight cancellations: we'll flash the repeating fractions
        # Collect the pieces that cancel so we can animate them
        cancel_parts = [
            *expanded_terms[0][7:10],   # 1/2 in first group
            *expanded_terms[1][5:8],    # +1/2 in second group
            *expanded_terms[1][9:12],   # 1/3 in second group
            *expanded_terms[2][5:8],    # +1/3 in third group
        ]  # type: ignore[arg-type]

        self.play(*(c.animate.set_color(RED).set_opacity(0.6) for c in cancel_parts), run_time=0.5)
        self.play(*(FadeOut(c) for c in cancel_parts), run_time=0.6)  # mimic cancellation

        # Fade the whole expanded line slightly to shift focus
        self.play(expanded_terms.animate.set_opacity(0.35), run_time=0.3)

        ######################
        # 4) Partial-sum formula
        ######################
